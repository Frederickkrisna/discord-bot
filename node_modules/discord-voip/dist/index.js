...
getNode("raw" /* Raw */).addEdge({
  ...
  cost: 1.5,
  transformer: () => new import_opus.OpusEncoder({ rate: 48e3, channels: 2, frameSize: 960 })
});
getNode("opus" /* Opus */).addEdge({
  type: "opus decoder" /* OpusDecoder */,
  to: getNode("raw" /* Raw */),
  cost: 1.5,
  transformer: () => new import_opus.OpusDecoder({ rate: 48e3, channels: 2, frameSize: 960 })
});
...
var FFMPEG_PCM_EDGE = {
  ...
  to: getNode("raw" /* Raw */),
  cost: 2,
  transformer: (input) => new import_ffmpeg.FFmpeg({
...
function canEnableFFmpegOptimizations() {
  try {
    return import_ffmpeg.FFmpeg.locate().metadata.includes("--enable-libopus");
  } catch {
  }
  return false;
}
...
function inferStreamType(stream) {
  if (stream instanceof import_opus2.OpusEncoder || stream instanceof import_prism_media2.default.opus.Encoder) {
    return { streamType: "opus" /* Opus */, hasVolume: false };
  } else if (stream instanceof import_opus2.OpusDecoder || stream instanceof import_prism_media2.default.opus.Decoder) {
...
function generateDependencyReport() {
  ...
  report.push("FFmpeg");
  try {
    const info = import_ffmpeg2.FFmpeg.locate();
...
async function demuxProbe(stream, probeSize = 1024, validator = validateDiscordOpusHead) {
  return new Promise((resolve2, reject) => {
    ...
    const webm = new import_prism_media3.default.opus.WebmDemuxer();
    webm.once("error", noop);
    webm.on("head", foundHead("webm/opus" /* WebmOpus */));
    const ogg = new import_prism_media3.default.opus.OggDemuxer();
    ogg.once("error", noop);

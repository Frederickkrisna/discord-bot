...
import { OpusEncoder as OpusEncoder2, OpusDecoder as OpusDecoder2 } from "@discord-player/opus";
...
getNode("raw" /* Raw */).addEdge({
  ...
  cost: 1.5,
  transformer: () => new OpusEncoder({ rate: 48e3, channels: 2, frameSize: 960 })
});
getNode("opus" /* Opus */).addEdge({
  type: "opus decoder" /* OpusDecoder */,
  to: getNode("raw" /* Raw */),
  cost: 1.5,
  transformer: () => new OpusDecoder({ rate: 48e3, channels: 2, frameSize: 960 })
});
...
function canEnableFFmpegOptimizations() {
  try {
    return FFmpeg.locate().metadata.includes("--enable-libopus");
  } catch {
  }
  return false;
}
  ...
  constructor(edges, streams, metadata, silencePaddingFrames) {
    ...
    for (const stream of streams) {
      if (stream instanceof prism2.VolumeTransformer) {
        this.volume = stream;
      } else if (stream instanceof OpusEncoder2) {
        this.encoder = stream;
      }
    }
...
function inferStreamType(stream) {
  if (stream instanceof OpusEncoder2 || stream instanceof prism2.opus.Encoder) {
    return { streamType: "opus" /* Opus */, hasVolume: false };
  } else if (stream instanceof OpusDecoder2 || stream instanceof prism2.opus.Decoder) {
...
import { FFmpeg as FFmpeg2 } from "@discord-player/ffmpeg";
...
async function demuxProbe(stream, probeSize = 1024, validator = validateDiscordOpusHead) {
  return new Promise((resolve2, reject) => {
    ...
    const webm = new prism3.opus.WebmDemuxer();
    webm.once("error", noop);
    webm.on("head", foundHead("webm/opus" /* WebmOpus */));
    const ogg = new prism3.opus.OggDemuxer();
    ogg.once("error", noop);
